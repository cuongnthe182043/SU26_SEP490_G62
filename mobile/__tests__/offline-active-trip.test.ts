import { decideOnError } from '@/hooks/use-active-trip';
...
// thuần `decideOnError` và kiểm thử trực tiếp ở đây.
// ─────────────────────────────────────────────────────────────────────────────
...
describe('MỨC 3 — Quy tắc dùng dữ liệu đệm cho màn chuyến', () => {
    ...
    it('G62-AT-01: mất mạng VÀ có đệm → dùng dữ liệu đệm, đánh dấu ngoại tuyến', () => {
        ...
        expect(decideOnError(loiMang, true)).toBe('ngoai-tuyen');
    });

    it('G62-AT-02: mất mạng nhưng CHƯA có đệm → phải báo lỗi, không có gì để hiện', () => {
        const loiMang = new ApiError('Không có kết nối mạng', 0);

        expect(decideOnError(loiMang, false)).toBe('bao-loi');
    });
    ...
    it('G62-AT-03: lỗi 403 dù CÓ đệm vẫn phải báo lỗi — không được giả vờ ngoại tuyến', () => {
        ...
        expect(decideOnError(loiQuyen, true)).toBe('bao-loi');
    });

    it('G62-AT-04: lỗi 500 dù có đệm vẫn báo lỗi', () => {
        expect(decideOnError(new ApiError('Lỗi máy chủ', 500), true)).toBe('bao-loi');
    });

    it('G62-AT-05: lỗi 401 (hết phiên) dù có đệm vẫn báo lỗi để app cho đăng nhập lại', () => {
        expect(decideOnError(new ApiError('Phiên hết hạn', 401), true)).toBe('bao-loi');
    });

    it('G62-AT-06: lỗi thường (không phải ApiError) → báo lỗi', () => {
        expect(decideOnError(new Error('Lỗi lập trình'), true)).toBe('bao-loi');
    });

    it('G62-AT-07: đệm giữ đủ thông tin tài xế cần ở hiện trường', async () => {
        await offlineCache.write('active-trip', CHUYEN);

        const ban = await offlineCache.read<typeof CHUYEN>('active-trip');
    ...
    it('G62-AT-08: server báo không còn chuyến → đệm phải ghi null, không giữ chuyến cũ', async () => {
        ...
        await offlineCache.write('active-trip', CHUYEN);
        await offlineCache.write('active-trip', null);

        const ban = await offlineCache.read<typeof CHUYEN | null>('active-trip');
