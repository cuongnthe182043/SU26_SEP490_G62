...
export const STATUS_BANNER: Partial<Record<TripStatus, { icon: React.ReactNode; text: string }>> = {
    claimed: {
        icon: <Package size={14} color={appTheme.colors.primary} />,
        text: 'Di chuyển đến điểm lấy hàng',
    },
    picking: {
        icon: <Truck size={14} color={appTheme.colors.warningText} />,
        text: 'Chụp ảnh lấy hàng để xác nhận và bắt đầu vận chuyển',
    },
    transit: {
        icon: <Navigation size={14} color={appTheme.colors.primary} />,
        text: 'Đang vận chuyển đến điểm giao',
    },
    arrived: {
        icon: <MapPin size={14} color={appTheme.colors.success} />,
        text: 'Đã đến - chụp ảnh biên lai rồi hoàn thành',
    },
    failed: {
        icon: <XCircle size={14} color={appTheme.colors.danger} />,
        text: 'Giao hàng thất bại - bắt đầu hoàn hàng về',
    },
    returning: {
        icon: <RotateCcw size={14} color={appTheme.colors.textMuted} />,
        text: 'Đang hoàn hàng về điểm lấy hàng ban đầu',
    },
};
