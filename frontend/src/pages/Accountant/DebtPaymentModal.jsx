  ...
  return (
    <div className="mt-2">
      <Button
        ...
        className="text-gray-500 dark:text-gray-400 justify-start"
      >
      ...
      {expanded && (
        <div className="mt-2 flex flex-col gap-2 max-h-72 overflow-auto">
          {loading ? (
            ...
            <p className="text-center text-gray-400 dark:text-gray-400 text-sm py-3">Chưa có lịch sử thanh toán</p>
          ) : histories.map((payment) => (
            <div key={payment.id} className="p-3 border border-gray-200 dark:border-white/10 rounded-xl">
              <div className="flex items-center gap-3">
                <span className="flex-1 text-[13px] font-medium text-gray-500 dark:text-gray-400">{formatDate(payment.paid_at)}</span>
                <span className="text-sm font-bold text-green-600 dark:text-green-300">{fmt(payment.total_amount || payment.totalAmount)}đ</span>
                <span>{payment.payment_method === 'cash' ? <RiMoneyDollarCircleLine size={15} /> : <RiBankLine size={15} />}</span>
              </div>
              {payment.items?.length > 0 && (
                <div className="mt-2 pt-2 border-t border-gray-100 dark:border-white/10 flex flex-col gap-0.5">
                  {payment.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between text-[12px] text-gray-400 dark:text-gray-400">
              ...
              {payment.notes && <p className="mt-1 text-[12px] text-gray-400 dark:text-gray-400 italic">{payment.notes}</p>}
            </div>
          ))}
        </div>
      )}
          ...
          {(onClose) => (
            <>
              ...
              <ModalBody className="py-4 flex flex-col gap-4">
                {}
                <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/10">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-emerald-500 rounded-xl flex items-center justify-center text-white">
                      {person?.type === 'customer' ? <RiBuildingLine size={20} /> : <RiTruckLine size={20} />}
                    </div>
                    <div>
                      <p className="font-bold text-gray-800 dark:text-gray-100 text-sm">{person?.name}</p>
                      {person?.phone && (
                        <p className="text-[12px] text-gray-400 dark:text-gray-400 flex items-center gap-1">
                  ...
                  <div className="flex gap-5 text-right">
                    <div>
                      <p className="text-[11px] text-gray-400 dark:text-gray-400 font-bold uppercase">Tổng nợ</p>
                      <p className="text-base font-extrabold text-gray-800 dark:text-gray-100">{fmt(totalRemaining)}đ</p>
                    </div>
                    <div>
                      <p className="text-[11px] text-gray-400 dark:text-gray-400 font-bold uppercase">Đã chọn</p>
                ...
                {}
                {debts.length > 0 && (
                  <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/10 flex flex-col gap-3">
                    <div className="flex items-end gap-2 flex-wrap">
                      <Input
                        ...
                        startContent={<RiMoneyDollarCircleLine size={16} className="text-gray-400 dark:text-gray-400 shrink-0" />}
                        endContent={<span className="text-gray-400 dark:text-gray-400 text-sm">đ</span>}
                ...
                {loading && debts.length === 0 ? (
                  ...
                  <div className="text-center py-10">
                    ...
                    <p className="font-bold text-gray-700 dark:text-gray-200">Không có công nợ nào</p>
                    <p className="text-gray-400 dark:text-gray-400 text-sm">Tất cả khoản nợ đã được thanh toán!</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-2">
                    <p className="text-[11px] font-bold text-gray-400 dark:text-gray-400 uppercase tracking-wider">Danh sách khoản nợ</p>
                    {debts.map((debt) => {
                      ...
                      return (
                        <div key={debt.id} className={`p-4 rounded-2xl border-2 transition-all ${isSelected ? 'border-success bg-success-50/30' : 'border-gray-200 dark:border-white/10 bg-white dark:bg-[#161922]'}`}>
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <div className="flex-1">
                              <p className="text-sm font-bold text-gray-800 dark:text-gray-100">#{debt.shipment_id || debt.id} - {debt.order_cargo_name || 'Chuyến xe'}</p>
                          ...
                          <div className="flex gap-5 mb-3">
                            {[['Tổng nợ', debt.total_amount, ''], ['Đã trả', debt.paid_amount, 'text-success'], ['Còn nợ', debtRemaining, 'text-danger']].map(([lbl, val, cls]) => (
                              <div key={lbl}>
                                <p className="text-[11px] text-gray-400 dark:text-gray-400 font-bold uppercase">{lbl}</p>
                                <p className={`text-sm font-bold ${cls || 'text-gray-800 dark:text-gray-100'}`}>{fmt(val)}đ</p>
                              </div>
                            ))}
                          </div>
                ...
                {debts.length > 0 && (
                  <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/10 flex flex-col gap-3">
                    <p className="text-[11px] font-bold text-gray-400 dark:text-gray-400 uppercase tracking-wider">Thông tin thanh toán</p>
                    ...
                    <Input
                      ...
                      startContent={<RiFileTextLine size={16} className="text-gray-400 dark:text-gray-400 shrink-0" />}
                    />

                    {}
                    <div className="p-3 bg-white dark:bg-[#161922] border-2 border-success-400 rounded-xl flex flex-col gap-1">
                      {[
                        ...
                        <div key={lbl} className="flex justify-between items-center text-sm text-gray-500 dark:text-gray-400">
                ...
                {preview?.preview?.map((item, i) => (
                  <div key={i} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-white/5 rounded-xl">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-gray-800 dark:text-gray-100">#{item.debtId} - {item.orderCargoName || 'Khoản nợ'}</span>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[11px] text-gray-400 dark:text-gray-400">Còn: {fmt(item.remaining - item.allocateAmount)}đ</p>
                    </div>
                  </div>
                ))}
            ...
            <>
              ...
              <ModalBody className="flex flex-col gap-1">
                {[
                  ...
                  <div key={lbl} className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-white/10 last:border-0 text-sm">
                    <span className="text-gray-400 dark:text-gray-400">{lbl}</span>
                    <span className="font-semibold text-gray-800 dark:text-gray-100">{val}</span>
                  </div>
                ))}
                <p className="text-[12px] text-gray-400 dark:text-gray-400 text-center mt-2">Hành động này không thể hoàn tác.</p>
              </ModalBody>
              <ModalFooter>
          ...
          {() => (
            <>
              <ModalHeader className="flex flex-col items-center gap-2 pt-6">
                ...
                <span className="text-lg font-extrabold text-gray-800 dark:text-gray-100">Thu tiền thành công!</span>
                <span className="text-3xl font-black text-success">{fmt(successData?.totalAmount)}đ</span>
              </ModalHeader>
              <ModalBody className="flex flex-col gap-2">
                <p className="text-[11px] font-bold text-gray-400 dark:text-gray-400 uppercase tracking-wider">Chi tiết phân bổ:</p>
                {successData?.allocations?.map((item, i) => (
                  <div key={i} className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-white/10 last:border-0">
                    <div className="flex items-center gap-2">
