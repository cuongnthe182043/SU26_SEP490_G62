...
-- ─── MIGRATION: thêm cột cho debt_payments (driver tự báo nộp tiền) ───────────
...
-- ─── MIGRATION: xóa bảng bills nếu vẫn còn ──────────────────────────────────
...
BEGIN
    ...
    IF v_completed_shipment_id IS NOT NULL AND NOT EXISTS (
        ...
        -- Tạo driver debt (khách trả tiền mặt → driver cầm tiền → sinh công nợ ngay)
        ...
        VALUES ('driver', v_driver_id, v_completed_shipment_id, v_order_id, 1500000, 500000, 'partial',
                'Khách thanh toán tiền mặt khi nhận hàng — driver đang cầm tiền');

        -- Lần nộp tiền đầu (đã được kế toán xác nhận → paid_amount += 500k)
        INSERT INTO debt_payments (debt_id, amount, payment_method, status, receipt_url, confirmed_at, created_by, notes, paid_at)
        SELECT d.id, 500000, 'cash', 'confirmed',
               'https://res.cloudinary.com/demo/image/upload/sample.jpg',
               NOW() - INTERVAL '10 hours',
               v_driver_id, 'Nộp tiền mặt tại văn phòng', NOW() - INTERVAL '12 hours'
        FROM debts d WHERE d.driver_id = v_driver_id AND d.total_amount = 1500000;

        -- Lần nộp tiền thứ hai (đang chờ kế toán xác nhận)
        INSERT INTO debt_payments (debt_id, amount, payment_method, status, receipt_url, created_by, notes, paid_at)
        SELECT d.id, 700000, 'bank_transfer', 'pending',
               'https://res.cloudinary.com/demo/image/upload/sample2.jpg',
               v_driver_id, 'Chuyển khoản 700k còn lại', NOW() - INTERVAL '1 hour'
        FROM debts d WHERE d.driver_id = v_driver_id AND d.total_amount = 1500000;

        -- Thêm 1 debt khác (unpaid hoàn toàn — để test danh sách)
