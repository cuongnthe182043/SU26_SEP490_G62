...
-- Tài xế/xe hiện tại của mỗi chuyến = dòng history mới nhất.
-- to_driver_id IS NULL (release_to_pool) => chuyến đang ở pool, không ai giữ.
-- Chuyến chưa từng có history => không xuất hiện ở đây (LEFT JOIN trả NULL).
