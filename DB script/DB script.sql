...
-- Phiếu chi thủ công (khoản chi ngoài vận hành: văn phòng, thuê kho, điện nước...)
-- Quy trình: Accountant tạo (pending) → Manager duyệt (approved/rejected) → Accountant chi (paid, ghi sổ FT)
CREATE TABLE payment_vouchers (
    id               INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    voucher_type     TEXT NOT NULL CHECK (voucher_type IN ('office','rent','utilities','equipment','entertainment','compensation','other')),
    amount           NUMERIC(12,2) NOT NULL CHECK (amount > 0),
    payee            TEXT NOT NULL,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Chốt kỳ báo cáo kinh doanh (Phase 3) — immutable snapshot + ký duyệt.
-- Mỗi (năm, tháng) chốt 1 lần, lưu nguyên bản báo cáo vào snapshot (JSONB) để số
-- không đổi khi có giao dịch phát sinh về sau. Xem alter_business_report_periods.sql.
-- ─────────────────────────────────────────────────────────────────────────────
