...
CREATE TABLE shipment_receipts (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    shipment_id         INT NOT NULL REFERENCES order_shipments(id) ON DELETE CASCADE,
    -- Hình thức thanh toán coordinator xác nhận khi tạo phiếu thu
    ...
    qr_code_data        TEXT,
    -- Driver xác nhận thực tế thu tiền theo hình thức nào
...
-- Thông tin công ty — luôn có đúng 1 row (id = 1)
...
-- ─── MIGRATION: chạy trên DB live đã tồn tại ────────────────────────────────
...
--         CHECK (payment_type IN ('cash_collected','bank_transfer','client_credit','qr_transfer')),
--     ADD COLUMN IF NOT EXISTS driver_collection_type TEXT
--         CHECK (driver_collection_type IN ('cash_collected','bank_transfer','client_credit','qr_transfer')),
