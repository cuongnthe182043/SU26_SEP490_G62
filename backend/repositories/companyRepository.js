...
const getCompanyInfo = async () => {
    const result = await pool.query(
        `SELECT id, company_name, tax_code, address, hotline, bank_name, bank_account_number, bank_account_name, bank_qr_url, updated_at
         FROM company_info WHERE id = 1`,
    );
    return result.rows[0] ?? null;
};

const upsertCompanyInfo = async ({ companyName, taxCode, address, hotline, bankName, bankAccountNumber, bankAccountName, updatedBy }) => {
    const result = await pool.query(
        `INSERT INTO company_info (id, company_name, tax_code, address, hotline, bank_name, bank_account_number, bank_account_name, updated_at, updated_by)
         VALUES (1, $1, $2, $3, $4, $5, $6, $7, NOW(), $8)
         ON CONFLICT (id) DO UPDATE SET
             company_name        = COALESCE(EXCLUDED.company_name,        company_info.company_name),
             tax_code            = COALESCE(EXCLUDED.tax_code,            company_info.tax_code),
         ...
         RETURNING *`,
        [
            companyName ?? null, taxCode ?? null, address ?? null, hotline ?? null,
            bankName ?? null, bankAccountNumber ?? null, bankAccountName ?? null, updatedBy,
        ],
    );
