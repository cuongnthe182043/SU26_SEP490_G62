...
const getFinanceStats = async () => {
    ...
    const query = `
        ...
        outstanding_debts AS (
            SELECT
                COALESCE(SUM(GREATEST(d.total_amount - COALESCE(dp_agg.paid_amount, 0), 0)), 0) AS total_receivables,
                COUNT(DISTINCT d.order_id)
                    FILTER (WHERE (d.total_amount - COALESCE(dp_agg.paid_amount, 0)) > 0.01) AS pending_count
            FROM debts d
            LEFT JOIN (
